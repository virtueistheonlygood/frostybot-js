...
module.exports = class frostybot_exchange_binance_margin extends frostybot_exchange_binance_base {
    ...
    async initialize() {
        this.type = 'margin'                         // Exchange subtype
        this.shortname = 'binance_margin'            // Abbreviated name for this exchange
        this.description = 'Binance Margin'          // Full name for this exchange
    ...
    async markets() {
        /*
        var marginpairs = [];
        var marginpairsresult = await this.execute('sapiGetMarginAllPairs');
        if (Array.isArray(marginpairsresult)) {
            for(var i = 0; i < marginpairsresult.length; i++) {
                marginpairs.push(marginpairsresult[i].symbol);
            }
        }
        */
        var cached = this.mod.exchange.markets({exchange: 'binance_margin'})
        ...
        if (this.mod.utils.is_array(raw_markets))
            raw_markets
            .filter(raw_market => raw_market.active == true /*&& marginpairs.includes(raw_market.id)*/)
            .forEach(raw_market => {
